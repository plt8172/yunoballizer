...
Each post is saved to its own downloaded/instagram/<account>/<shortcode>/
...
def harvest(
    ...
    if accounts is None:
        accounts = config.input_values("instagram")
        if not accounts:
            logger.info("No Instagram accounts configured, skipping")
            return
    ...
    for account in accounts:
        ...
        try:
            ...
            def include_post(post: object) -> bool:
                ...
                return not post_is_complete(post_dir, post.mediacount)
