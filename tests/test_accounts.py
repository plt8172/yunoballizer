...
class AccountsTests(unittest.TestCase):
    def test_unknown_platform_is_rejected(self) -> None:
        with self.assertRaises(SystemExit):
            accounts.list_accounts("mastodon")

    def test_add_normalizes_handles_and_dedupes(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(accounts.config, "CONFIG_DIR", config_dir):
                ...
                self.assertTrue(accounts.add("youtube", "@NASA"))

                stored = json.loads((config_dir / "inputs.json").read_text(encoding="utf-8"))

            self.assertEqual(stored["instagram"], ["nasa"])
            self.assertEqual(stored["youtube"], ["nasa"])

    def test_remove_drops_only_the_matching_account(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.object(accounts.config, "CONFIG_DIR", Path(tmpdir)):
    ...
    def test_hand_edited_handles_are_normalized_on_next_write(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir)
            (config_dir / "inputs.json").write_text(
                json.dumps({"instagram": [], "youtube": ["@MrBeast"], "tiktok": [], "urls": []}),
                encoding="utf-8",
            )
            with patch.object(accounts.config, "CONFIG_DIR", config_dir):
                self.assertFalse(accounts.add("youtube", "mrbeast"))
                self.assertTrue(accounts.add("youtube", "Veritasium"))

            stored = json.loads((config_dir / "inputs.json").read_text(encoding="utf-8"))
            self.assertEqual(stored["youtube"], ["mrbeast", "veritasium"])
